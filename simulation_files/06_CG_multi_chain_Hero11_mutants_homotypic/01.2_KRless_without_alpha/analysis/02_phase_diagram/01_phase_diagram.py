...
def main():
    ...
    num_run = 5
    MD_T_list = [210 + i * 10 for i in range(num_temperature)]
    ...
    for j in range(num_temperature_meaningful):
        for k in range(num_run):
            fname = "../density_evolution/hero11_noalpha_100_md1_T_{0:0>2d}_{1:0>2d}_density_averaged.dat".format(j + 1, k + 1)
            if k == 0:
