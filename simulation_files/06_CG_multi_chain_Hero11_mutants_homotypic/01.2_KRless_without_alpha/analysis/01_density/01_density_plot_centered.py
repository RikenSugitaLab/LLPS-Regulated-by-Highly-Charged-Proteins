...
def main():
    ...
    # ==========
    # trajectory name
    sim_name = "hero11_noalpha_100_md1"

    # simulation temperatures
    T_list = [210 + i * 10 for i in range(10)]
    ...
    for i, t in enumerate(T_list):
        ...
        for i_run in range(num_run):
            ...
            for j in range(plot_steps):
                ...
                new_index = [k for k in range(i_center - (num_bins // 2), i_center + (num_bins // 2))]
                data_shift = arr_tmp[new_index]
                new_data[j, :] = data_shift[:]

                # calculate high density and low density
                lo_dens_region = [k for k in range(i_center - (num_bins // 2), i_lo_bound - 1)] + [k for k in range(i_hi_bound + 2, i_center + (num_bins // 2))]
            ...
            axes[i * num_run + i_run].set_yticks([0, (num_bins // 2), num_bins])
            axes[i * num_run + i_run].set_yticklabels([0, box_size_z // 2, box_size_z], fontsize=10)
            axes[i * num_run + i_run].set_ylim(0, num_bins)
