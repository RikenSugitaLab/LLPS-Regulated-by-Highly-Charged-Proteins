...
def main():
    T_list = [90 + i * 10 for i in range(8)]
    ...
    for i, t in enumerate(T_list):
        for i_run in range(num_run):
            print("Temperature:", t)
            fname = "hero11_noalpha_100_md1_scrmb_T_{1:0>2d}_{0:0>2d}.density.dat".format(i_run + 1, i + 1)
            data_local = np.loadtxt(fname)[::step_interval, :]

            density_profile_output_fname = "hero11_scrmb_{0:d}_md1_T_{1:0>2d}_density_centered.dat".format(i_run + 1, i + 1)
            density_average_output_fname = "hero11_scrmb_{0:d}_md1_T_{1:0>2d}_density_averaged.dat".format(i_run + 1, i + 1)
            ...
            axes[i * num_run + i_run].imshow(new_data.T, cmap="Blues", vmin=0.000001, vmax=.005, origin="lower")
            ...
            axes[i * num_run + i_run].set_yticklabels([0, 15, 30], fontsize=10)
            axes[i * num_run + i_run].set_ylim(0, 100)
            axes[i * num_run + i_run].set_ylabel(r"z ($10^2\AA$)", fontsize=12)

            sub_leg = "T={0:>3d} K, scramble no. {1:d}".format(t, i_run)
    ...
    figname = "hero11_scrambles_md1_T_density_centered.png".format(t)
