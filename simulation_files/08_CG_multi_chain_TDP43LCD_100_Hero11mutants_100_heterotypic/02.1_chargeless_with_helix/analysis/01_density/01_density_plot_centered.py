...
def main():
    ...
    T_list = [160 + i * 10 for i in range(20)]
    ...
    for i, t in enumerate(T_list):
        print("Temperature:", t)
        fname0 = "{0}_md1_T_{1:0>2d}.density.dat".format(pro0_name, i + 1)
        fname1 = "{0}_md1_T_{1:0>2d}.density.dat".format(pro1_name, i + 1)
        data0_local = np.loadtxt(fname0)[::step_interval, :]
        data1_local = np.loadtxt(fname1)[::step_interval, :]

        pro0_density_profile_output_fname = "{0}_md1_T_{1:0>2d}_density_centered.dat".format(pro0_name, i + 1)
        pro0_density_average_output_fname = "{0}_md1_T_{1:0>2d}_density_averaged.dat".format(pro0_name, i + 1)
        pro1_density_profile_output_fname = "{0}_md1_T_{1:0>2d}_density_centered.dat".format(pro1_name, i + 1)
        pro1_density_average_output_fname = "{0}_md1_T_{1:0>2d}_density_averaged.dat".format(pro1_name, i + 1)
    ...
    axes[num_temperatures * 2 - 1].set_xlim(0, 1000)
    axes[num_temperatures * 2 - 1].set_xlabel(r"MD steps ($\times 10^6$)", fontsize=12)

    figname = "{0}_{1}_md1_T_density_centered.png".format(pro0_name, pro1_name)
