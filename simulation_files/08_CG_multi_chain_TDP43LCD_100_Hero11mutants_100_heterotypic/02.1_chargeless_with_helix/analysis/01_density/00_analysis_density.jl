...
for (iT, T) in enumerate(T_list)
    for i in 1 : n_run
        ...
        if_name = @sprintf("../../%s_%s_md1_T_%02d.dcd", NAME_PRO1, NAME_PRO2, iT)
        println("Processing ", if_name)

        mytraj = read_dcd(if_name)

        # EDIT THIS: output data file name
        of_pro1_name = @sprintf("%s_md1_T_%02d.density.dat", NAME_PRO1, iT)
        of_pro2_name = @sprintf("%s_md1_T_%02d.density.dat", NAME_PRO2, iT)
