...
@everywhere function calculate_MSD(temperature, i_conc, i_md, i_run)
    ...
    for i_Δt in 1:n_Δt
        ...
        if length(square_displacement_list) > 0
        ...
        if length(square_displacement_list) > 0
            pro2_MSD_dense_list[i_Δt] = sum(square_displacement_list) / length(square_displacement_list)
        else
            pro2_MSD_dense_list[i_Δt] = -1
        end
        # ----------------
        # dilute diffusion
        # ----------------
        ...
        if length(square_displacement_list) > 0
            pro1_MSD_dilute_list[i_Δt] = sum(square_displacement_list) / length(square_displacement_list)
        else
            pro1_MSD_dilute_list[i_Δt] = -1
        end
        # pro2
        square_displacement_list = []
        for i_chain in 1:num_pro1
    ...
    pro1_MSD_fname = @sprintf("%s_T%3d_md%d_r%02d.%s_MSD.dat", system_name, temperature, i_md, i_run, name_pro1)
    ...
    pro2_MSD_fname = @sprintf("%s_T%3d_md%d_r%02d.%s_MSD.dat", system_name, temperature, i_md, i_run, name_pro2)
