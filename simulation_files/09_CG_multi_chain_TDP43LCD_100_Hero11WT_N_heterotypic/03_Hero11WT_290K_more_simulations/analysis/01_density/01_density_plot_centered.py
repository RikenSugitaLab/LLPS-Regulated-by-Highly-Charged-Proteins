...
def compute_plot_density(conc_hero11):
    ...
    T_list = [290 + i * 5 for i in range(2)]
    ...
    fig, axes = plt.subplots(num_temperatures * 2, num_run * 1, figsize=(num_run * 9, num_temperatures * 2), constrained_layout=False, sharex=False, sharey=False)
    for i, t in enumerate(T_list):
        for i_run in range(num_run):
            system_name = "{0}_{1}_{2}_{3}".format(pro0_name, 100, pro1_name, conc_hero11)
            print("Temperature:", t)
            md1_fname0 = "md1/{0}_T{1:0>3d}_md1_r{3:0>2d}.{2}.density.dat".format(system_name, t, pro0_name, i_run + 1)
            md1_fname1 = "md1/{0}_T{1:0>3d}_md1_r{3:0>2d}.{2}.density.dat".format(system_name, t, pro1_name, i_run + 1)
            md1_data0_local = np.loadtxt(md1_fname0)[::step_interval, :]
            md1_data1_local = np.loadtxt(md1_fname1)[::step_interval, :]

            md2_fname0 = "md2/{0}_T{1:0>3d}_md2_r{3:0>2d}.{2}.density.dat".format(system_name, t, pro0_name, i_run + 1)
            md2_fname1 = "md2/{0}_T{1:0>3d}_md2_r{3:0>2d}.{2}.density.dat".format(system_name, t, pro1_name, i_run + 1)
            md2_data0_local = np.loadtxt(md2_fname0)[::step_interval, :]
            md2_data1_local = np.loadtxt(md2_fname1)[::step_interval, :]

            md3_fname0 = "md3/{0}_T{1:0>3d}_md3_r{3:0>2d}.{2}.density.dat".format(system_name, t, pro0_name, i_run + 1)
            md3_fname1 = "md3/{0}_T{1:0>3d}_md3_r{3:0>2d}.{2}.density.dat".format(system_name, t, pro1_name, i_run + 1)
            ...
            pro0_density_profile_output_fname = "{0}_T{1:0>3d}_r{3:>02d}_{2}_density_centered.dat".format(system_name, t, pro0_name, i_run + 1)
            pro0_density_average_output_fname = "{0}_T{1:0>3d}_r{3:>02d}_{2}_density_averaged.dat".format(system_name, t, pro0_name, i_run + 1)
            pro1_density_profile_output_fname = "{0}_T{1:0>3d}_r{3:>02d}_{2}_density_centered.dat".format(system_name, t, pro1_name, i_run + 1)
            pro1_density_average_output_fname = "{0}_T{1:0>3d}_r{3:>02d}_{2}_density_averaged.dat".format(system_name, t, pro1_name, i_run + 1)
            ...
            axes[i * 2, i_run].imshow(pro0_plot_data.T, cmap="Blues", vmin=0.000001, vmax=.005, origin="lower")
            axes[i * 2 + 1, i_run].imshow(pro1_plot_data.T, cmap="Reds", vmin=0.000001, vmax=.005, origin="lower")

            axes[i * 2, i_run].set_xticks([500 * j for j in range(11)])
            axes[i * 2, i_run].set_xlim(0, 3000)
            axes[i * 2, i_run].set_xticklabels([15 * j for j in range(11)], fontsize=10)
            axes[i * 2 + 1, i_run].set_xticks([500 * j for j in range(11)])
            axes[i * 2 + 1, i_run].set_xlim(0, 3000)
            axes[i * 2 + 1, i_run].set_xticklabels([15 * j for j in range(11)], fontsize=10)

            axes[i * 2, i_run].set_yticks([0, 50, 100])
            axes[i * 2, i_run].set_yticklabels([0, 15, 30], fontsize=10)
            axes[i * 2, i_run].set_ylim(0, 100)
            axes[i * 2, i_run].set_ylabel(r"z ($10^2\AA$)", fontsize=12)
            axes[i * 2 + 1, i_run].set_yticks([0, 50, 100])
            axes[i * 2 + 1, i_run].set_yticklabels([0, 15, 30], fontsize=10)
            axes[i * 2 + 1, i_run].set_ylim(0, 100)
            axes[i * 2 + 1, i_run].set_ylabel(r"z ($10^2\AA$)", fontsize=12)

            sub_leg_0 = "T={0:>3d} K, {1}".format(t, pro0_name)
            sub_leg_1 = "T={0:>3d} K, {1}".format(t, pro1_name)
            axes[i * 2, i_run].text(100, 75, sub_leg_0)
            axes[i * 2 + 1, i_run].text(100, 75, sub_leg_1)

    # axes[num_temperatures * 2 - 1, i_run].set_xticks([500 * j for j in range(11)])
    # axes[num_temperatures * 2 - 1, i_run].set_xticklabels([15 * j for j in range(11)], fontsize=10)
    # axes[num_temperatures * 2 - 1, i_run].set_xlim(0, 3000)
    # axes[num_temperatures * 2 - 1, i_run].set_xlabel(r"MD steps ($\times 10^6$)", fontsize=12)
