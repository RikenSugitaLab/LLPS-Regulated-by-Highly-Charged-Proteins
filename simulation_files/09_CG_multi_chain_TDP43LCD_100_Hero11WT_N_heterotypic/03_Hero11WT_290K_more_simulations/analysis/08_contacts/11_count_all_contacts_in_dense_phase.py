...
def count_dense_contacts(T, n_hero):
    n_run = 5
    ...
    for i_run in range(n_run):
        # hero11-hero11
        cnt_00 = []
        fname0 = "./md2/tdp43_100_hero11_{2}_T{0}_md2_r{1:0>2d}.hero11-hero11.dense.contact_count_ts.dat"
        ...
        # hero11-tdp43
        cnt_01 = []
        fname0 = "./md2/tdp43_100_hero11_{2}_T{0}_md2_r{1:0>2d}.tdp43-hero11.dense.contact_count_ts.dat"
        fname = fname0.format(T, i_run + 1, n_hero)
        with open(fname, "r") as fin:
            for line in fin:
                tmp_sum = 0
                ...
                for w in words:
                    ...
                    if cnt >= 0:
                        tmp_sum += cnt
                cnt_01.append(tmp_sum)

        # tdp43-tdp43
        cnt_11 = []
        fname0 = "./md2/tdp43_100_hero11_{2}_T{0}_md2_r{1:0>2d}.tdp43-tdp43.dense.contact_count_ts.dat"
        fname = fname0.format(T, i_run + 1, n_hero)
        with open(fname, "r") as fin:
            for line in fin:
                tmp_sum = 0
    ...
    cnt_01_mean = np.mean(cnt_01_flat)
    cnt_01_std  = np.std(cnt_01_flat)
    cnt_01_err  = cnt_01_std / np.sqrt(len(cnt_01_flat))
    ...
    fout_name = "T{0}_nHero_{1}_contact_all_MD2_REVISION.dat".format(T, n_hero)
    fout = open(fout_name, "w")
    fout.write("ALL: {0:12.3f}   {1:12.3f}   {2:12.3f} \n".format(cnt_sum_mean, cnt_sum_std, cnt_sum_err))
    fout.write("0-0: {0:12.3f}   {1:12.3f}   {2:12.3f} \n".format(cnt_00_mean, cnt_00_std, cnt_00_err))
    fout.write("0-1: {0:12.3f}   {1:12.3f}   {2:12.3f} \n".format(cnt_01_mean, cnt_01_std, cnt_01_err))
    fout.write("1-1: {0:12.3f}   {1:12.3f}   {2:12.3f} \n".format(cnt_11_mean, cnt_11_std, cnt_11_err))
    fout.close()

if __name__ == '__main__':
    for n_hero in [i* 10 + 10 for i in range(9)]:
        count_dense_contacts(290, n_hero)
