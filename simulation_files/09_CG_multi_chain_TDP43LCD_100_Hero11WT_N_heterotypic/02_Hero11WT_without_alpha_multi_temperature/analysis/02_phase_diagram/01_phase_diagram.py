...
def main(conc_hero11):
    system_name = "tdp43_100_hero11_{0}".format(conc_hero11)
    pro0_name = "tdp43"
    num_temperature = 7
    ...
    for j in range(num_temperature_meaningful):
        fname = "../01_density_in_box/{0}_T_{1:0>3d}_{2}_density_averaged.dat".format(system_name, MD_T_list[j], pro0_name)
        density_data = np.loadtxt(fname, usecols=(3, 4))[6000:, :]
    ...
    ax.set_ylim(T_min, 325)
    ax.set_ylabel("temperature (K)", fontsize=16)

    figname = "{0}_phase_diagram_fitted.svg".format(system_name)
    plt.savefig(figname)
    # plt.show()
    ...
    pd_raw_fname = "{0}_phase_diagram_raw.dat".format(system_name)
    pd_fit_fname = "{0}_phase_diagram_fit.dat".format(system_name)
