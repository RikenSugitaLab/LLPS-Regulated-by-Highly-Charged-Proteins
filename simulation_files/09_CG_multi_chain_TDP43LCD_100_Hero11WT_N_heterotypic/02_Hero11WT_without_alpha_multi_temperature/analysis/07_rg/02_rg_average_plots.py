...
def main():
    ...
    color_list = ["r", "g", "b", "m"]
    rg_0_list = [33.168, 33.194, 33.182, 33.078]
    for i_T, T in enumerate(T_list):
        ...
        for n in n_list:
            ...
            for j in [2, 3]:
                ...
                pro1_rg = np.loadtxt(rg_data_fname.format(j, system_name, T_list[i_T], pro1_name))
                pro2_rg = np.loadtxt(rg_data_fname.format(j, system_name, T_list[i_T], pro2_name))

                # load position data
                pos_data_fname = "../03_molecular_position/md{0}/{1}_t{2}_md{0}.{3}_pos.dat"
                pro1_pos = np.loadtxt(pos_data_fname.format(j, system_name, T_list[i_T], pro1_name))
                pro2_pos = np.loadtxt(pos_data_fname.format(j, system_name, T_list[i_T], pro2_name))
    ...
    axes[1].set_yticks([31.5, 32.0, 32.5])
    axes[1].set_yticklabels(["31.5", "32.0", "32.5"], fontsize=12)
    axes[1].set_ylim(31.05, 32.75)
