...
def calculate_partition_from_density(temperature, i_run):
    ...
    for tstep in range(pro2_data.shape[0]):
        # p1_data = pro1_data[tstep]
        p2_data = pro2_data[tstep]
        data_size = p2_data.size
        ...
        p2_in_count  = sum(p2_data[j] for j in range(46, 55))
        p2_out_count = sum(p2_data[j] for j in range(100) if (j < 46 or j >= 55))
