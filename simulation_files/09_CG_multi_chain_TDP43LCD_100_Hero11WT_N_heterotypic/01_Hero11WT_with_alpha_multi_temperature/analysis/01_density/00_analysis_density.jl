...
for (iT, T) in enumerate(T_list)
    for i in 1 : n_conc
        ...
        if_name = @sprintf("../../%s_%d_%s_%d_T%03d_md3.dcd", NAME_PRO1, 100, NAME_PRO2, i * 10, T)
        println("Processing ", if_name)

        mytraj = read_dcd(if_name)

        # EDIT THIS: output data file name
        of_pro1_name = @sprintf("%s_%d_%s_%d_md3_T_%03d.%s.density.dat", NAME_PRO1, 100, NAME_PRO2, i * 10, T, NAME_PRO1)
        of_pro2_name = @sprintf("%s_%d_%s_%d_md3_T_%03d.%s.density.dat", NAME_PRO1, 100, NAME_PRO2, i * 10, T, NAME_PRO2)
