...
def main(phase_state):
    ...
    # data_name = "./md3_1d_count/tdp43_1d_contact_count.tdp43-{0}.{1}.T{2}.hero11_{3}.dat"
    data_name = "./md3_analysis_with_frame_average/tdp43_100_hero11_{3}_t{2}_md3.tdp43-{0}.{1}.contact_count_ts.dat"
    # data_name = "./check/tdp43_100_hero11_{3}_t{2}_md3.tdp43-{0}.{1}.contact_count_ts.dat"
    color_list = ["r", "g", "b", "m"]
    for i_T, T in enumerate(T_list):
        ...
        for n in n_list:
            y1 = []
            y2 = []
            fname = data_name.format(name_pro1, phase_state, T, n)
                ...
                for line in fin:
                    ...
                    if len(words) > 0:
                        for w in words:
                            if float(w) >= 0:
                                y1.append(float(w))
            fname = data_name.format(name_pro2, phase_state, T, n)
            with open(fname, "r") as fin:
                for line in fin:
                    words = line.split()
                    if len(words) > 0:
    ...
    # o_data_name = "/home/ctan/Port/HD1/HERO/TDP43/20220206_first_test/24_tdp43_100_multi_temperature_260_350_HPS-Urry/analysis/contact_map/inter-chain/tdp43_contact_count_1d_T{0:>02d}.chain_pairwise.dat"
    # o_data_name = "/home/ctan/Port/HD1/HERO/TDP43/20220206_first_test/25_tdp43_100_multi_temperature_260_350_HPS-Urry_FIX_BONDLENGTH/analysis/contact_map/inter-chain/tdp43_contact_count_1d_T{0:>02d}.chain_pairwise.dat"
    ...
    of_tmp = open(output_fname, "w")
    for i_T, T in enumerate(T_list):
        fname = o_data_name.format(i_T + 1, phase_state)
